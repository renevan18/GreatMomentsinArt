...
@interface FlappyScene() <SKPhysicsContactDelegate>
@end
...
- (void)setupPlayerAnimation {
  ...
  // #2: Good
  SKTextureAtlas *atlas = [SKTextureAtlas atlasNamed:@"flappy_sprites"];
...
- (void)switchToNewGame:(GameState)state {
  ...
  SKScene *newScene = [[FlappyScene alloc] initWithSize:self.size delegate:_sceneDelegate state:state];
...
- (void)shareScore {
  ...
  [self.sceneDelegate shareString:initialTextString url:url image:screenshot];
}
